...
from astar_dynamic import astar_dynamic, Node, is_path_affected, compute_path_from, merge_paths, find_affected_segment
...
def pathfinding_node():
    ...
    while not rospy.is_shutdown():
        if grid is not None and robot_position is not None:
            ...
            if new_path:
                if not is_path_affected(new_path, grid):
                    # If the new path is not affected, set it as the current path
                ...
                if affected_start is not None:
                    # Calculate left and right segments
                    left_segment = current_path[:affected_start]
                    affected_segment = current_path[affected_start:affected_end + 1]
                    right_segment = current_path[affected_end + 1:]

                    # Calculate new path segment for the affected part
                    new_path_segment = compute_path_from(grid, Node(None, affected_segment[0]), end)
                    if new_path_segment:
                        # Merge the paths: left + new affected segment + right
                        current_path = left_segment + new_path_segment + right_segment
                        publish_path(current_path, path_publisher)
                    else:
                        rospy.loginfo("No valid path found from the current position.")
