...
def map_callback(data):
    global grid
    data = map_inflation.inflate_map(data)
    # grid = [[data.data[(j*4000) + i] for j in range(0, 4000)] for i in range(0, 4000)]
    grid = [[d[i] for d in data] for i in range(0, 4000)]
...
def merge(ps):
    if ps[0][0] == ps[1][0] == ps[2][0] or ps[1][1] == ps[0][1] == ps[2][1]:
        return [ps[0], ps[2]]
    if abs(ps[0][0] - ps[2][0]) == abs(ps[0][1] - ps[2][1]):
        return [ps[0], ps[2]]
    return ps
...
def smooth_current_path(path):
    ...
    while not(done):
        ...
        for i in (1, len(new_path)):
            j = i + 1
            k = i + 2
            if j < len(new_path) and k < len(new_path):
                ps = merge(new_path[i:i+3])
                if len(ps) != 3:
                    done = True
                for p in ps:
                    c_path.append(p)
            elif j < len(path):
                c_path.append(path[j])
                c_path.append(path[i])
            else: 
                c_path.append(i)
        new_path = c_path
        c_path = []
        ...
        if grid is not None and robot_position is not None:
            ...
            if current_path is None:
                ...
                if new_path:
                    ...
                    current_path = smooth_current_path(current_path)
